...
class DOMIT_Node {
	...
	*/
	function getText() {
		return UTF8toMamboCharset($this->nodeValue);
	} //getText
...
class DOMIT_Document extends DOMIT_ChildNodes_Interface {
	...
	*/
	function getTextFromFile($filename) {
		...
		$rx = '/<\?xml.*encoding=[\'\"](\S*)[\'\"].*\?>/i';
		$m = array();
		if (preg_match($rx, $fileContents, $m)) {
		...
		if(!($encoding == "UTF-8" || $encoding == "US-ASCII" || $encoding == "ISO-8859-1")) {
			$encoded_fileContents = mos_convert_encoding($fileContents, "UTF-8", $encoding);

			if($encoded_fileContents != NULL) {
				$fileContents = str_replace ( $m[0],'<?xml version="1.0" encoding="utf-8"?>', $encoded_fileContents);
			}
		}
	...
	*/
	function getAttribute($name) {
		...
		if ($returnNode == null) {
			...
			return UTF8toMamboCharset($returnNode->getValue());
		}
	} //getAttribute
...
class DOMIT_ProcessingInstruction extends DOMIT_Node {
	...
	*/
	function getText() {
		return ($this->nodeName . ' ' . UTF8toMamboCharset($this->nodeValue));
	} //getText
...
class DOMIT_DocumentType extends DOMIT_Node {
	...
	*/
	function getText() {
		return UTF8toMamboCharset($this->text);
	} //getText
		...
		if ((strlen($data) > 2)  && ($this->parseItem == '')){
			...
			switch ($pre) {
				...
				case '<![': //cdata section coming
					...
					break;	
				case '<!-': //comment
					$this->commentElement($this, substr($data, 4, (strlen($data) - 7)));	
					break;
				case '<!D': //doctype
