...
function saveMambot( $option, $client, $task ) {
	...
	switch ( $task ) {
		case 'apply':
			$msg = $adminLanguage->A_COMP_MAMB_SAVED_CHANGES . $row->name;
			mosRedirect( 'index2.php?option='. $option .'&client='. $client .'&task=editA&hidemainmenu=1&id='. $row->id, $msg );

		case 'save':
		default:
			$msg = $adminLanguage->A_COMP_MAMB_SAVED . $row->name;
...
function editMambot( $option, $uid, $client ) {
	...
	if ($uid) {
		...
		$lists['ordering'] = '<input type="hidden" name="ordering" value="'. $row->ordering .'" />' . $adminLanguage->A_COMP_MAMB_ORDERING;
	}
