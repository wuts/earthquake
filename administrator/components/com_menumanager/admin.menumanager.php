...
function saveMenu() {
	...
	if ( $old_menutype == 'mainmenu' ) {
		if ( $menutype <> 'mainmenu' ) {
			echo "<script> alert('$adminLanguage->A_COMP_MENU_RENAME_WARNING'); window.history.go(-1); </script>\n";
			exit;			
		}
	}
	...
	foreach ( $menus as $menu ) {
		...
		if ( $params->menutype == $menutype ) {
			echo "<script> alert('$adminLanguage->A_COMP_MENU_EXISTS_WARNING'); window.history.go(-1); </script>\n";
			exit;
		}
	}
	...
	switch ( $new ) {
		...
		// create a new module for the new menu
			...
			$msg = $adminLanguage->A_COMP_MENU_CREATED . ' [ '. $menutype .' ]';			
			break;
