...
function editCategory( $uid=0, $section='' ) {
	...
	if ($uid) {
		...
		if ( $row->section > 0 ) {
			...
			. "\n AND ( type = 'content_blog_category' OR type = 'content_category' )"
			;
			$database->setQuery( $query );
	...
	if ($row->section == 'com_contact') {
		$types[] = mosHTML::makeOption( 'contact_category_table', $adminLanguage->A_COMP_CATEG_CONTACT_CATEG_TABLE );
	} else
	if ($row->section == 'com_newsfeeds') {
		$types[] = mosHTML::makeOption( 'newsfeed_category_table', $adminLanguage->A_COMP_CATEG_NEWSFEED_CATEG_TABLE );
	} else
	if ($row->section == 'com_weblinks') {
		$types[] = mosHTML::makeOption( 'weblink_category_table', $adminLanguage->A_COMP_CATEG_WEBLINK_CATEG_TABLE );
	} else {
		$types[] = mosHTML::makeOption( 'content_category', $adminLanguage->A_COMP_CATEG_CONTENT_CATEG_TABLE );
		$types[] = mosHTML::makeOption( 'content_blog_category', $adminLanguage->A_COMP_CATEG_CONTENT_CATEG_BLOG );
	} // if
